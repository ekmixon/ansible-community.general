...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _query(self):
        ...
        for stack_slug in self.stack_slugs:
            try:
                workloads = self._stackpath_query_get_list(
                    f'{self.api_host}/workload/v1/stacks/{stack_slug}/workloads'
                )

            except Exception:
                raise AnsibleError(
                    f"Failed to get workloads from the StackPath API: {traceback.format_exc()}"
                )
            ...
            for workload in workloads:
                try:
                    ...
                    raise AnsibleError(
                        f"Failed to get workload instances from the StackPath API: {traceback.format_exc()}"
                    )
    ...
    def _stackpath_query_get_list(self, url):
        ...
        while next_page:
            resp = open_url(
                url + f'?page_request.first=10&page_request.after={cursor}',
                headers=headers,
                method="GET",
            )
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not self.stack_slugs:
            try:
                ...
                raise AnsibleError(
                    f"Failed to get stack IDs from the Stackpath API: {traceback.format_exc()}"
                )
